...
import com.android.justordinarymovieapp.databinding.ItemMovie2Binding
...
class MovieAdapter2(val context: Context) :
    ...
    inner class ViewHolder(private val binding: ItemMovie2Binding) :
        RecyclerView.ViewHolder(binding.root) {
        fun bindView(data: MovieResponse) {
            with(binding) {
                var date = data.releaseDate?.substring(0, 4)
                tvTitle.text = String.format("%s (%s)", data.title, date)
                Glide.with(context)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return when (viewType) {
            1 -> ViewHolder(parent.viewBinding(ItemMovie2Binding::inflate))
